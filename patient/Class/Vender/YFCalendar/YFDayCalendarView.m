...
#import "YFDayComponentView.h"
#import "YFDayIndicatorView.h"

@interface YFDayCalendarView (){
...
@property (strong, nonatomic) YFDayNavigationBar *navigationBar;
...
@property (strong, nonatomic) YFDayIndicatorView *selectedIndicatorView;
@property (strong, nonatomic) YFDayIndicatorView *todayIndicatorView;
@property (strong, nonatomic) NSMutableArray<YFDayComponentView *> *componentViews;
...
- (void)commonInit {
    ...
    NSDateComponents *comps = [YFDayUnits dateComponentsFromDate:todayDate];
    ...
    self.navigationBar = [[YFDayNavigationBar alloc] init];
...
- (void)setSelectedDate:(NSDate *)selectedDate {
    NSDateComponents *comps = [YFDayUnits dateComponentsFromDate:selectedDate];
    int64_t delayTime = 0;
...
- (void)makeUIElements {
    ...
    self.selectedIndicatorView = [[YFDayIndicatorView alloc] init];
    self.selectedIndicatorView.translatesAutoresizingMaskIntoConstraints = NO;
    self.selectedIndicatorView.hidden = YES;
    self.todayIndicatorView = [[YFDayIndicatorView alloc] init];
    ...
    for (int i = 0; i < 42; i++) {
        YFDayComponentView *componentView = [[YFDayComponentView alloc] init];
...
- (void)configureWeekdayHeaderView {
    ...
    [self.weekHeaderView.arrangedSubviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (canUseLocalizedStrings) {
            ...
            weekdayLabel.text = [YFDayUnits stringOfWeekdayInChinese:idx + 1];
        }
    }];
}

- (void)configureComponentView:(YFDayComponentView *)view withDay:(NSUInteger)day month:(NSUInteger)month year:(NSUInteger)year {
    NSDateComponents *comps = [[NSDateComponents alloc] init];
    comps.day = day;
    comps.month = month;
    comps.year = year;
    
    if ([YFDayUnits isDateTodayWithDateComponents:comps]) {
        ...
        if (self->_currentVisibleRow == 0) {
            NSUInteger paddingDays = [YFDayUnits firstWeekdayInMonth:self->_visibleMonth ofYear:self->_visibleYear] - 1;
    ...
    if (self.showUserEvents) {
        [self->_eventsInVisibleMonth enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            EKEvent *event = obj;
            if ([[YFDayUnits dateFromDateComponents:comps] isEqualToDate:event.startDate]) {
...
- (void)configureContentView {
    ...
    NSUInteger totalDays = [YFDayUnits daysInMonth:self->_visibleMonth ofYear:self->_visibleYear];
    NSUInteger paddingDays = [YFDayUnits firstWeekdayInMonth:self->_visibleMonth ofYear:self->_visibleYear] - 1;
    ...
    if (self.showUserEvents) {
        ...
        NSPredicate *predicate = [self.eventStore predicateForEventsWithStartDate:[YFDayUnits dateFromDateComponents:startComps]
                                                                          endDate:[YFDayUnits dateFromDateComponents:endComps]
                                                                        calendars:nil];
    ...
    NSUInteger totalDaysInLastMonth = [YFDayUnits daysInMonth:paddingMonth ofYear:paddingYear];
...
//    NSString *stringOfMonth = [YFDayUnits stringOfMonthInEnglish:self->_visibleMonth];
//    return [NSString stringWithFormat:@"%@ %lu", stringOfMonth, (unsigned long) self->_visibleYear];
    return [NSString stringWithFormat:@"%lu-%lu", (unsigned long) self->_visibleYear, (unsigned long) self->_visibleMonth];
}

- (YFDayComponentView *)componentViewForDateComponents:(NSDateComponents *)comps {
    __block YFDayComponentView *view = nil;
    [self.componentViews enumerateObjectsUsingBlock:^(YFDayComponentView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)navigationBarButtonDidTap:(id)sender {
    switch (self.navigationBar.lastCommand) {
        case YFDayNavigationBarCommandPrevious:
            [self jumpToPreviousMonth];
            break;
            
        case YFDayNavigationBarCommandNext:
...
- (void)componentDidTap:(YFDayComponentView *)sender {
    ...
    if (self.selectedIndicatorView.hidden || self.selectedIndicatorView.alpha == 0) {
        ...
        [UIView animateWithDuration:0.3 delay:0 usingSpringWithDamping:0.8 initialSpringVelocity:0 options:kNilOptions animations:^{
            ...
            [((YFDayComponentView *) self.selectedIndicatorView.attachingView) setSelected:NO];
            [sender setSelected:YES];
        } completion:nil];
    ...
    self->_selectedDate = [YFDayUnits dateFromDateComponents:comps];
...
- (void)jumpToPreviousMonth {
    if (self.singleRowMode) {
        if (self->_currentVisibleRow > 0) {
            ...
            // by Rakuyo. Optimize jump logic
            NSUInteger totalDays = [YFDayUnits daysInMonth:self->_visibleMonth - 1 ofYear:self->_visibleYear];
            NSUInteger paddingDays = [YFDayUnits firstWeekdayInMonth:self->_visibleMonth - 1  ofYear:self->_visibleYear] - 1;
